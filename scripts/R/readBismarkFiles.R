# This code has been taken from https://gist.github.com/al2na/4839e615e2401d73fe51
                               ...
                               context="CpG",min.cov=10)
{
  ...
  for(i in 1:length(location)){
    ...
    result[[i]]= new("methylRaw",data.frame(chr=df[,1],start=df[,2],end=df[,2],
                                            strand=df[,3],coverage=(df[,5]+df[,6]),
    ...
    #result[[i]]= new("methylRaw",data.frame(chr=df[,1],start=df[,3],end=df[,3],
    #                                        strand=df[,2],coverage=(df[,4]+df[,5]),
    ...
    #                 resolution="base"
    #)
    #result[[i]]= data.frame(chr=df[,1],start=df[,2],end=df[,2],
    #                                        strand=df[,3],coverage=(df[,5]+df[,6]),
    #                                        numCs=df[,5],numTs=df[,6])
...
readBismarkCoverage_col45 <-function(location,sample.id,assembly="unknown",treatment,
                                   context="CpG",min.cov=10){
  if(length(location)>1){
    stopifnot(length(location)==length(sample.id),
              length(location)==length(treatment))
